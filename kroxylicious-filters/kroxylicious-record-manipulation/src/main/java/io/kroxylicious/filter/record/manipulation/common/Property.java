...
public interface Property<S, A> {
     ...
     * @param s the whole structure
     * @param f the transformation, given the current (possibly absent) value and a {@link OpContext}
     ...
     */
    default S modify(S s, BaseTypedOp<Maybe<A>, Maybe<A>> f, OpContext opContext) {
        return switch (f.apply(get(s), opContext)) {
